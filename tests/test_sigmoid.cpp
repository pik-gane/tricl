...
int main ()
{
    ...
    // reference values:
    check(near_abs(probunits2probability(1, 0, 0), 0.731058578630, 1e-11), "(0,0) pu=1 -> expit(1)");
    check(near_rel(probunits2probability(-50, 0, 1), 2.6e-26, 0.05), "(0,1) pu=-50 -> 2.6e-26");
    check(near_abs(probunits2probability(50, 0, 1), 0.991653, 1e-6), "(0,1) pu=+50 -> 0.991653");
    check(near_abs(probunits2probability(3, 1e-9, 1), 0.876248035533, 1e-10), "(1e-9,1) pu=3");
    check(near_abs(probunits2probability(3, 0, 1), 0.876248035498, 1e-10), "(0,1) pu=3");
    check(near_abs(probunits2probability(3, 1e-9, 1) - probunits2probability(3, 0, 1), 0.0, 1e-8), "continuity at zero tail index");

    // slope 1/4 at zero probability units, by finite differences and analytically:
    for (auto tails : std::vector<std::pair<double, double>> { {0, 0}, {1, 1}, {2, 0.5}, {0, 1}, {1, 0}, {5, 0.2}, {10, 10} }) {
        ...
        check(near_abs(slope_fd(0, l, r), 0.25, 1e-6), buf);
        std::snprintf(buf, sizeof buf, "analytic derivative at 0 for tails (%g,%g)", l, r);
        check(near_abs(probunits2probability_derivative(0, l, r, tail2scale(l) + tail2scale(r)), 0.25, 1e-12), buf);
        // limits:
    ...
    // exactly the expit function for tail indices (0,0):
    for (double pu = -30; pu <= 30; pu += 0.5) {
        std::snprintf(buf, sizeof buf, "expit at pu=%g", pu);
        check(near_rel(probunits2probability(pu, 0, 0), 1 / (1 + std::exp(-pu)), 1e-14), buf);
    }
    ...
    for (double l : tail_grid) {
        for (double r : tail_grid) {
            ...
            for (double pu = -100; pu <= 100; pu += 0.25) {
                ...
                if ((pu > -20) && (pu <= 20)) check(p > last, buf);
                last = p;
